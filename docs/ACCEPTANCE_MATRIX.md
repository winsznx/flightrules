...
| A9 | The failure links to real SigNoz trace evidence | `packages/signoz-mcp/src/web-url.ts`, `apps/api/src/routes/violation-evidence.ts`, `apps/web` violation inspector | `web-url.test.ts` (10), `evidence-summary.test.ts` (8), `phase-15-violation-inspector.spec.ts` (30) | phase-14 and phase-15 — SL-061 found every stored trace URL was null and why; links are now built from SigNoz's own verified path and the operator's origin. All three critical violations audited live from rule to trace to downstream metric | DONE |
...
| A12 | FlightRules creates an alert that fires from the v2 violation | `packages/artifact-compiler`, `apps/worker`, `scripts/verify-alert-lifecycle.mjs` | `compile.test.ts` alert suite, `phase-10.signoz.integration.test.ts`, `make verify-alerts` | `docs/evidence/phase-10-result.md` and `docs/evidence/phase-16/alert-lifecycle.md` — a canary evaluation drove `Violation Rate Alert` to `firing` at value 220 and `Duplicate Side Effect Alert` at value 22, and **both recovered to `inactive` exactly 300 s later**, matching the configured `evalWindow`. Every query and threshold is byte-identical before and after the cycle | DONE |
...
| A15 | A clean clone can reproduce the system | `scripts/verify-fresh-machine.sh`, `scripts/bootstrap-signoz.sh`, `Makefile`, `README.md` | `make verify-fresh-machine`, `scripts/verify-reproducibility.sh` | `docs/evidence/phase-16/fresh-machine.txt` — a clone outside the working tree, from committed files only, with the SigNoz and application volumes destroyed first: Foundry deploy, lock reproduces byte-identically, bootstrap, credential, migrations, build, 90 s idle survival, `make demo-full` exit 0 then exit 2, ten artefacts read-back verified | DONE |
...
| 6 | Instrumented refund-agent demo emits traces, metrics and logs | `apps/demo-*`, `packages/telemetry` | `packages/telemetry/src/telemetry.test.ts`, `packages/telemetry/src/instruments.ts`, `make verify-telemetry` | phase-04 traces; phase-10 metric series; phase-16 `otlp-logs.md` — logs are exported over OTLP and correlate to the trace that produced them, including two `refund committed to the payment ledger` lines in one failing trace | DONE |
...
| 19 | Release comparison shows baseline versus canary topology diff | `packages/trace-graph/src/diff.ts`, `apps/api/src/routes/diff.ts`, `apps/web` graph diff | `diff` unit tests (50), `gate.integration.test.ts` diff suite (7), `phase-14-release-diff.spec.ts` | phase-14 — the live endpoint returns 15 typed changes for `refund-agent-v2`, naming the missing `policy.retrieve`, the missing `fraud.check` and the duplicated `payment.refund` write | DONE |
| 20 | Violation inspector links to the original SigNoz trace | `apps/web` violation inspector, `packages/signoz-mcp/src/web-url.ts` | `phase-15-violation-inspector.spec.ts` | phase-15 — the link resolves to `${SIGNOZ_URL}/trace/<traceId>`, the path SigNoz's own trace-details answer uses | DONE |
...
| 24 | Unit, property, integration, e2e, security and reproducibility tests pass | whole repository | `make verify`, `make test-integration`, `make test-e2e`, `make verify-fresh-machine` | phase-16 — 1,415 unit, 286 integration and 86 browser tests against the live product, five security scans at exit 0, and a fresh-clone reproduction through both gate exit codes | DONE |
...
| FR-017 | Evidence linking | `packages/db`, `apps/api`, `packages/signoz-mcp/src/web-url.ts`, `apps/web` | `api.integration.test.ts`, `web-url.test.ts`, `evidence-summary.test.ts`, `phase-15-violation-inspector.spec.ts` | phase-15 — every stored trace URL was null until SL-061; the inspector now shows rule, trace, spans, canonical nodes, approved comparison, correlated logs, downstream metrics and a safe copyable summary | DONE |
...
| FR-020 | Demo reset | `apps/api`, `scripts/reset-demo.sh`, `scripts/purge-managed-artifacts.mjs` | `api.integration.test.ts` — reset, reseed, demo-mode guard; `phase-13-workflow.spec.ts` resets live before the exit gate | phase-13 and phase-16 — a clean start is a managed-artefact purge **then** the reset; without the purge the next sync correctly reports ten conflicts because the SigNoz resources outlive the register rows. A clone with destroyed volumes needs neither, which the fresh-machine reproduction exercises | DONE |
...
| `/projects/[projectId]/agents/[agentId]/releases` | Releases list | `apps/web/src/app/projects/[projectId]/agents/[agentId]/releases/page.tsx` | `web.test.ts`; `phase-14-release-diff.spec.ts` ten columns and decision filters; live smoke | DONE |
| `/projects/[projectId]/agents/[agentId]/releases/[releaseId]` | Release Diff | `.../releases/[releaseId]/page.tsx`, `components/graph-diff.tsx`, `evidence/route.ts` | `web.test.ts`; `phase-14-release-diff.spec.ts` exit gate; live smoke | DONE |
